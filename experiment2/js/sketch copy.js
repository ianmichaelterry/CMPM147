...
function draw() {
  ...
  let topColor = color(0, 76, 153);
  let bottomColor = color(135, 206, 250);
  setGradient(0, 0, width, height / 2, topColor, bottomColor);
  ...
  for (let i = grassBlades.length - 1; i >= 0; i--) {
      let blade = grassBlades[i];
      blade.x += 1; // Move grass to the right
      if (blade.x > width + 20) { // Remove grass that moves out of the canvas
          grassBlades.splice(i, 1);
      } else {
          if (blade.x < leftmost) leftmost = blade.x;
          drawGrass(blade.x, blade.baseY, blade.height, blade.noiseOffset, wind);
      }
  }

  // Add new grass blades if there's visible space
  if (leftmost > 20) { // Dynamic blade addition threshold
      addGrassBlade();
  }
}
...
function initializeGrass() {
  let numBlades = 300; // Set a sufficient number to cover the entire width
  let spacing = width / numBlades; // Calculate even spacing
  for (let i = 0; i < numBlades; i++) {
      addGrassBlade(true, i * spacing); // Place each blade at an even interval
  }
}



// Update drawGrass to draw from the top of the canal
...
function addGrassBlade(initial = false, xPos = 0) {
  let xPosition = initial ? xPos : random(-50, 0); // Adjust starting position for dynamic additions
  let canalTop = height - (height * 0.1); // Top of the canal
  grassBlades.push({
      x: xPosition,
      height: random(50, 100),
      baseY: canalTop, // Position the base at the top of the canal
      noiseOffset: random(1000)
  });
}
